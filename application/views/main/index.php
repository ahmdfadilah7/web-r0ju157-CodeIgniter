<div class="container">
	...
	<div class="row my-2">
		...
		<div class="col-lg-3">
			<div class="card shadow">
			  <div class="card-body">
			    <h6><i class="fas fa-fw fa-dollar-sign"></i> <br>Total Penjualan</h6>
			    <?php 
				    $pembayaran = $this->pm->getAllPembayaran();
			    	$penjualan = 0;
			    	foreach ($pembayaran as $dp) {
			    		if ($dp['status_bayar'] == 'sudah_dibayar') {
				    		$penjualan += $this->pm->totalHargaTerakhir($dp['kode_invoice'])['total_harga_terakhir'];
			    		} else {
			    			$penjualan += 0;
			    		}
			    	}
			    ?>
			    <h6 class="text-muted">Rp. <?= number_format($penjualan); ?></h6>
			  </div>
			</div>
		</div>
		<div class="col-lg-3">
			<div class="card shadow">
			  <div class="card-body">
			    <h6><i class="text-success fas fa-fw fa-caret-up"></i> <i class="text-success fas fa-fw fa-dollar-sign"></i> <br>Non Omset</h6>
			    <?php 
			    	$omset_modal = 0;
			    	foreach ($pembayaran as $dp) {
			    		if ($dp['status_bayar'] == 'sudah_dibayar') {
				    		$omset_modal += $this->pm->totalHargaTerakhir_modal($dp['kode_invoice'])['total_harga_terakhir'];
			    		} else {
			    			$omset_modal += 0;
			    		}
			    	}
			    ?>
			    <h6 class="text-muted">Rp. <?= number_format($omset_modal); ?></h6>
			  </div>
			</div>
		</div>
		<div class="col-lg-3">
...
<?php
	$notif = $omset - $jumlah_pengeluaran;
	if ($notif < 0) {
?>
	<div class="flashdata-failed" data-flashdata="Total keuangan minus"></div>
<?php
	}
?>